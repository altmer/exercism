defmodule TwelveDays do
  ...
  def verse(number),
    do: "On the #{Enum.fetch!(@numbers, number - 1)} day of Christmas my true love gave to me, #{all_gifts(number)}."

  defp all_gifts(1),
    do: "a Partridge in a Pear Tree"

  defp all_gifts(number) do
    @gifts
    |> Enum.slice(@length - number..-1)
    |> Enum.join(", ")
  end
